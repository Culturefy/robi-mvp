...
export default function ConsultationInlineForm({ leadCategory, icpScore, clientDetails, selectionsPayload, onNotify, onSuccess }: Props) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-8" id="submit-details">
      <div className="flex items-center gap-3 mb-3">
        <svg className="w-6 h-6 text-blue-900" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
        </svg>
        <h2 className="text-xl font-semibold text-gray-900">Submit Details</h2>
      </div>
      <p className="text-gray-600 mb-6">Share your details and we’ll follow up shortly.</p>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <label className="block text-sm text-gray-900 mb-1">First Name</label>
            <input type="text" name="firstName" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-900" required />
          </div>
          <div>
            <label className="block text-sm text-gray-900 mb-1">Last Name</label>
            <input type="text" name="lastName" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-900" required />
          </div>
        </div>
        <div>
          <label className="block text-sm text-gray-900 mb-1">Email</label>
          <input type="email" name="email" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-900" required />
        </div>
        <div>
        ...
        <div>
          <label className="block text-sm text-gray-900 mb-1">Company</label>
          <input type="text" name="company" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-900" placeholder="Optional" />
        </div>
        <div>
          <label className="block text-sm text-gray-900 mb-1">Job Title</label>
          <input type="text" name="jobTitle" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-900" placeholder="e.g., CFO" />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <label className="block text-sm text-gray-900 mb-1">City</label>
            <input type="text" name="city" required className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-900" placeholder="City" />
          </div>
          <div>
            <label className="block text-sm text-gray-900 mb-1">State</label>
            <select name="state" required className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-900">
        ...
        <div>
          <label className="block text-sm text-gray-900 mb-1">We will be needing your tax documents</label>
          <input name="attachments" type="file" multiple className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-900" accept=".pdf,.doc,.docx,.xls,.xlsx,.png,.jpg,.jpeg,.gif,.txt" />
        ...
        <button ref={submitRef} type="submit" className="w-full text-white font-semibold py-2.5 rounded-lg transition-colors bg-blue-900 hover:bg-blue-800">
          Submit
        </button>
      </form>
    </div>
